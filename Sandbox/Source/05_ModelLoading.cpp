 ...
 public:
  xlux::Bool Execute(const xlux::RawPtr<VertexOutData> dataIn,
    ...
    const auto lightIntensity =
        std::max(0.0f, lightDir.Dot(dataIn->normal * -1.0f));
    dataOut->Color[0] =
        texture->Sample(dataIn->texCoord) * (lightIntensity + 0.3f);
...
int main() {
  ...
  auto textureData = stbi_loadf(
      (xlux::utils::GetExecutableDirectory() + "/texture.png").c_str(), &tWidth,
      &tHeight, &tChannels, 3);
  auto texture =
  ...
  while (!Window::HasClosed()) {
    ...
    Window::SetTitle(
        "Xlux Engine [Model Loading] - Jaysmito Mukherjee - FPS: " +
        std::to_string(1.0f / deltaTime));
    // vertexShader->model = xlux::math::Mat4x4::RotateY(currTime * 0.5f);
    view = xlux::math::Mat4x4::LookAt(
        xlux::math::Vec3(dist * cos(currTime * 0.2f), 12.0f,
                         dist * sin(currTime * 0.2f)),
        xlux::math::Vec3(0.0f, 0.0f, 0.0f), xlux::math::Vec3(0.0f, 1.0f, 0.0f));
