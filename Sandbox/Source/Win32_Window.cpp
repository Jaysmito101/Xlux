...
struct Win32_GraphicsBuffer {
  ...
  Win32_GraphicsBuffer(xlux::I32 wd, xlux::I32 hgt,
                       xlux::Bool onlyMemory = false) {
    if (onlyMemory) {
      ...
      this->hbm = CreateDIBSection(hdcScreen, &bmi, DIB_RGB_COLORS,
                                   (void**)(&this->data), NULL, NULL);
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam,
  ...
  switch (message) {
    ...
    case WM_PAINT: {
      ...
      BitBlt(hdc, 0, 0, Window::GetWidth(), Window::GetHeight(), hdcMem, 0, 0,
             SRCCOPY);
      DeleteDC(hdcMem);
      EndPaint(hwnd, &ps);
      break;
    ...
    case WM_SIZE: {
      ...
      s_FrontBuffer = xlux::CreateRawPtr<Win32_GraphicsBuffer>(
          Window::GetWidth(), Window::GetHeight(), false);
      s_BackBuffer = xlux::CreateRawPtr<Win32_GraphicsBuffer>(
          Window::GetWidth(), Window::GetHeight(), false);

      xlux::EventManager<WindowResize, xlux::I32, xlux::I32>::Get()->RaiseEvent(
...
Window::Window(const std::string& title, int width, int height) {
  ...
  s_FrontBuffer =
      xlux::CreateRawPtr<Win32_GraphicsBuffer>(m_Width, m_Height, false);
  s_BackBuffer =
      xlux::CreateRawPtr<Win32_GraphicsBuffer>(m_Width, m_Height, true);

  s_Window = this;
}

void* Window::GetRawHandle() { return s_WindowHandle; }

xlux::Bool Window::HasClosed() { return s_HasClosed; }
...
void Window::SwapBuffer() {
  memcpy(s_FrontBuffer->data, s_BackBuffer->data,
         s_Instance->GetWidth() * s_Instance->GetHeight() * sizeof(xlux::U32));
...
void Window::Clear(xlux::F32 r, xlux::F32 g, xlux::F32 b, xlux::F32 a) {
  ...
  int pvr = (int)(r * 255.0f), pvg = (int)(g * 255.0f), pvb = (int)(b * 255.0f),
      pva = (int)(a * 255.0f);
  unsigned int pixel = RGBA(pvr, pvg, pvb, pva);
  for (int i = 0; i < wd * hgt; ++i) s_BackBuffer->data[i] = pixel;
}

void Window::SetPixel(xlux::F32 x, xlux::F32 y, xlux::F32 r, xlux::F32 g,
                      xlux::F32 b, xlux::F32 a) {
  int wd = Window::GetWidth(), hgt = Window::GetHeight();
