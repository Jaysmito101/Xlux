...
class HelloWorldFShader
 ...
 public:
  xlux::Bool Execute(const xlux::RawPtr<VertexOutData> dataIn,
    ...
    dataOut->Color[0] =
        texture->Sample(dataIn->texCoord) * lightIntensity * lightColor;
    return true;
  }
};
...
int main() {
  ...
  const auto vertices = std::vector<VertexInData>{
      // Front
      VertexInData(xlux::math::Vec3(-0.5f, -0.5f, 0.5f),
                   xlux::math::Vec3(0.0f, 0.0f, 1.0f),
  ...
  auto proj = xlux::math::Mat4x4::Perspective(
      ...
      100.0f);
  auto view = xlux::math::Mat4x4::LookAt(xlux::math::Vec3(2.0f, 2.0f, 2.0f),
                                         xlux::math::Vec3(0.0f, 0.0f, 0.0f),
                                         xlux::math::Vec3(0.0f, 1.0f, 0.0f));
  ...
  while (!Window::HasClosed()) {
    ...
    Window::SetTitle("Xlux Engine [Going 3D] - Jaysmito Mukherjee - FPS: " +
