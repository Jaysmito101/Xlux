...
enum ETextureType { TextureType_2D, TextureType_Cube };

enum ETexelFormat { TexelFormat_RGB, TexelFormat_RGBA, TexelFormat_Depth };
...
class XLUX_API ITexture {
 public:
  ...
  inline Bool IsColor() const {
    return GetFormat() == TexelFormat_RGB || GetFormat() == TexelFormat_RGBA;
  }

  inline Size GetPixelSize() const {
    switch (GetFormat()) {
      case TexelFormat_RGB:
        return 3;
      case TexelFormat_RGBA:
        return 4;
      case TexelFormat_Depth:
        return 1;
      default:
        throw std::runtime_error("Invalid texel format");
    }
  }
  ...
  inline Size GetSizeInBytes() const {
    return GetPixelCount() * GetPixelSize() *
           sizeof(F32);  // internally everything is stored as F32
  }
  ...
  virtual Bool GetPixel(U32 x, U32 y, U32 z, F32& r, F32& g, F32& b,
                        F32& a) const = 0;
  ...
  virtual math::Vec4 Sample(const math::Vec3& uvw) const {
    (void)uvw;
    return math::Vec4(1.0f, 0.0f, 1.0f, 1.0f);
  }
  ...
  virtual math::Vec4 SampleInterpolated(const math::Vec3& uvw) const {
    ...
    math::Vec3 uvw100 = {(u_floor + 1) / width, v_floor / height,
                         w_floor / depth};
    math::Vec3 uvw010 = {u_floor / width, (v_floor + 1) / height,
                         w_floor / depth};
    math::Vec3 uvw110 = {(u_floor + 1) / width, (v_floor + 1) / height,
                         w_floor / depth};
    math::Vec3 uvw001 = {u_floor / width, v_floor / height,
                         (w_floor + 1) / depth};
    math::Vec3 uvw101 = {(u_floor + 1) / width, v_floor / height,
                         (w_floor + 1) / depth};
    math::Vec3 uvw011 = {u_floor / width, (v_floor + 1) / height,
                         (w_floor + 1) / depth};
    math::Vec3 uvw111 = {(u_floor + 1) / width, (v_floor + 1) / height,
                         (w_floor + 1) / depth};
...
class XLUX_API Texture2D : public ITexture {
 public:
  ...
  Bool GetPixel(U32 x, U32 y, U32 z, F32& r, F32& g, F32& b,
                F32& a) const override;
