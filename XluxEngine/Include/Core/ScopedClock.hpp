...
class XLUX_API ScopedClock {
 public:
  ScopedClock(const String& name)
      : m_Name(name), m_StartTime(std::chrono::high_resolution_clock::now()) {}

  ~ScopedClock() {
    auto endTime = std::chrono::high_resolution_clock::now();
    auto duration =
        std::chrono::duration<float, std::milli>(endTime - m_StartTime).count();
  ...
  XLUX_FORCE_INLINE float Elapsed() const {
    auto endTime = std::chrono::high_resolution_clock::now();
    return std::chrono::duration<float, std::milli>(endTime - m_StartTime)
        .count();
  }
