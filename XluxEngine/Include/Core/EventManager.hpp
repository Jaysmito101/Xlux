...
class EventManager {
 public:
  ...
  void Subscribe(std::function<void(ParamsData...)> callback,
                 String callbackID = "") {
    if (callbackID.empty()) {
  ...
  void Unsubscribe(const String& callbackName) {
    ...
    if (it != m_EventSubscribers.end()) m_EventSubscribers.erase(it);
  }

  void UnsubscribeAll() { m_EventSubscribers.clear(); }
...
template <U32 Type, typename... ParamsData>
Scope<EventManager<Type, ParamsData...>>
    EventManager<Type, ParamsData...>::s_Instance = nullptr;
}  // namespace xlux