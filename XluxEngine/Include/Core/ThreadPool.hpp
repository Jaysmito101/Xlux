...
class IJob {
 public:
  ...
  virtual Bool Execute(JobPayload payload, JobResult& result,
                       Size m_ThreadID) = 0;
};
...
class PoolWorker {
 public:
  ...
  inline Bool HasJob() const { return !m_Jobs.empty(); }
  ...
  inline Bool HasResult() const { return !m_JobDone.IsEmpty(); }
  ...
  inline void SetSleepDuration(Size duration) { m_SleepDuration = duration; }
...
class ThreadPool {
 public:
  ...
  ~ThreadPool() {
    for (U32 i = 0; i < JobCount; ++i) delete m_Workers[i];
  }

  inline void WaitJobDone() {
    for (U32 i = 0; i < JobCount; ++i) m_Workers[i]->WaitJobDone();
  }

  inline U32 GetWorkerCount() const { return JobCount; }
