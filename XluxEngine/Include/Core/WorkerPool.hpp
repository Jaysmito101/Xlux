...
template <typename JobPayload, typename JobWorker>
  requires std::is_copy_constructible_v<JobPayload> &&
           CJobWorker<JobWorker, JobPayload>
class Worker {
 public:
  Worker(U32 id, RawPtr<JobWorker> jobFunction)
      : m_ID(id), m_JobFunction(jobFunction) {
  ...
  Bool IsAlive() const { return m_IsAlive.test(std::memory_order::relaxed); }

  U32 GetID() const { return m_ID; }

  U32 GetQueueSize() const { return m_JobQueue.Size(); }

  Bool IsIdle() const { return m_JobQueue.IsEmpty(); }
  ...
  void AddJob(const JobPayload& job) { m_JobQueue.Push(job); }
