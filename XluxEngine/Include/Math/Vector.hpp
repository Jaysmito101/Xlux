...
class Vec {
 public:
  ...
  static XLUX_FORCE_INLINE Vec<N, ValueTypeT> From(const Vec<M>& other, Args...
  args)
  {
  ...
  sizeof(other.m_Data));
                  ...
                  {
                          std::memcpy(result.m_Data, other.m_Data, std::clamp(M,
  Size(0), N) * sizeof(ValueTypeT)); ValueTypeT data[] = { args... };
    ...
    // else
    {
      if constexpr (sizeof...(args) == 0) {
        std::memcpy(m_Data, other.m_Data,
                    std::clamp(M, Size(0), N) * sizeof(ValueTypeT));
  ...
  XLUX_FORCE_INLINE Vec<N, ValueTypeT>& operator=(
      const Vec<N, ValueTypeT>& other) {
    std::memcpy(m_Data, other.m_Data, sizeof(m_Data));
    return *this;
  }
  ...
  XLUX_FORCE_INLINE ValueTypeT& operator[](Size index) { return m_Data[index]; }
  ...
  XLUX_FORCE_INLINE Vec<N, ValueTypeT> operator+(
      const Vec<N, ValueTypeT>& other) const {
  ...
  XLUX_FORCE_INLINE Vec<N, ValueTypeT> Cross(
      const Vec<N, ValueTypeT>& other) const {
    static_assert(N == 3 || N == 2,
                  "Cross product is only defined for 3D vectors");
    if constexpr (N == 2) {
      F32 result = m_Data[0] * other[1] - m_Data[1] * other[0];
      return Vec<2, ValueTypeT>(result, 0.0f);
    } else if constexpr (N == 3) {
      return Vec<3, ValueTypeT>(m_Data[1] * other[2] - m_Data[2] * other[1],
  ...
  XLUX_FORCE_INLINE void Normalize() { *this /= Length(); }

  XLUX_FORCE_INLINE ValueTypeT* RawData() { return m_Data; }

  XLUX_FORCE_INLINE const ValueTypeT* RawData() const { return m_Data; }
