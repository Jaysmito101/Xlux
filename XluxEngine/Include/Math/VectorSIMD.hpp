...
class Vec {
  static_assert(N <= 4,
                " Vec<N, ValueTypeT> SIMD version is only defined for N < 4");
 ...
 public:
  ...
  XLUX_FORCE_INLINE ValueTypeT LengthSquared() const { return Dot(*this); }
  ...
  XLUX_FORCE_INLINE Vec<N, ValueTypeT> Cross(
    ...
    if constexpr (N == 2) {
      F32 result = m_Data.m128_f32[0] * other.m_Data.m128_f32[1] -
                   m_Data.m128_f32[1] * other.m_Data.m128_f32[0];
      ...
      __m128 tmp1 =
          _mm_shuffle_ps(other.m_Data, other.m_Data, _MM_SHUFFLE(3, 1, 0, 2));
