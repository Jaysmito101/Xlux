...
void Logger::Shutdown() { delete s_Instance; }
...
Bool Logger::DetachFile(const String& filename) {
  ...
  auto it =
      std::find_if(m_AttachedFiles.begin(), m_AttachedFiles.end(),
                   [&](const auto& file) { return file.first == fullath; });
...
void Logger::Log(LogLevel level, const String& message) {
  ...
  const auto logMessage = "[" + logLevelString + "]" +
                          std::string(8 - logLevelString.size(), ' ') + "[" +
                          timeStr.str() + "]\t: " + message + "\n";
...
String Logger::GetLogLevelString(LogLevel level) const {
  switch (level) {
    ...
    case xlux::LogLevel::Fatal:
      return "FATA";
    case xlux::LogLevel::All:
      return "AL";
    case xlux::LogLevel::None:
      return "NONE";
    default:
      return "UNKNOWN";
  }
  ...
  switch (level) {
    ...
    case xlux::LogLevel::Info:
      SetConsoleTextAttribute(
          hConsole, FOREGROUND_INTENSITY | FOREGROUND_GREEN | FOREGROUND_BLUE);
      break;
    case xlux::LogLevel::Warning:
      SetConsoleTextAttribute(
    ...
    case xlux::LogLevel::Fatal:
      SetConsoleTextAttribute(
          hConsole, FOREGROUND_INTENSITY | FOREGROUND_RED | FOREGROUND_BLUE);
      break;
    default:
      SetConsoleTextAttribute(hConsole, FOREGROUND_INTENSITY | FOREGROUND_RED |
                                            FOREGROUND_GREEN | FOREGROUND_BLUE);
